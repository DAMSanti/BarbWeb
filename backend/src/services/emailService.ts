...
export async function sendPasswordChangedEmail(
  ...
  try {
    logger.info('Sending password changed notification email', { to });

    const { data: result, error } = await resend.emails.send({
      from: FROM_EMAIL,
      to: [to],
      subject: '🔐 Tu contraseña ha sido cambiada - Barbara & Abogados',
      html: getPasswordChangedTemplate(data),
    });

    if (error) {
      logger.error('Error sending password changed email', { error, to });
      throw error;
    }

    logger.info('Password changed email sent successfully', { to, emailId: result?.id });
    return result;
  } catch (error) {
...
function getPasswordChangedTemplate(data: {
  clientName: string;
  changedAt: Date;
}): string {
  const formattedDate = data.changedAt.toLocaleString('es-ES', {
    dateStyle: 'long',
    timeStyle: 'short',
  });
    ...
    <tr>
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
          ...
          <tr>
            <td style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); padding: 40px; text-align: center; border-radius: 12px 12px 0 0;">
              <h1 style="margin: 0; color: #ffffff; font-size: 28px; font-weight: bold;">🔐 Contraseña Cambiada</h1>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 40px;">
              ...
              <div style="background-color: #fef3c7; padding: 16px; border-radius: 8px; border-left: 4px solid #d97706;">
                <p style="margin: 0; color: #92400e; font-size: 13px;">
                  <strong>⚠️ ¿No fuiste tú?</strong> Si no realizaste este cambio, contacta inmediatamente con nosotros para proteger tu cuenta.
                </p>
              </div>
            </td>
          </tr>
